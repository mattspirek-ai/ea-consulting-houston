...
## D-013 · 2026-09-07 · Prospect list: signal-led sourcing and an eight-gate verification standard
**Decision:** ICP prospects live in `prospects/` and are produced by a two-stage pipeline: (1) parallel research segments defined by a *trigger signal or population* (currently hiring an EA; recently funded/acquired; growth-list honoree; boutique law firms; physician-owned practices; PE/family office/RIA; small energy companies; professional-services principals; DFW/Austin/San Antonio expansion), each writing `raw/<segment>.json` with a source URL on every field; (2) a deterministic verifier (`scripts/verify_prospects.py`) that re-fetches sources and applies eight gates — website live, decision-maker named on an official page, two independent title sources, phone found on the company's own pages, email published on an official page *and* MX present, trigger sourced, metro in scope, size band 5–150. A row is **Verified** only when all six hard gates pass; everything else is retained as Partially Verified or Rejected with reasons. Enrichment happens through `raw/patches/*.json` that only add sourced values or browser confirmations, never edit research. Reports (`verification-log.md`, `outreach-segments.md`) are generated by script; the log is append-only.
**Contact-data policy:** publicly published business channels only — company main lines, direct lines/emails printed on official bios or filings (Form ADV, State Bar, NPI), general inboxes, public LinkedIn URLs. No data brokers, no pattern-guessed emails, no personal cells or home addresses. "Verified email" means published + MX; the build environment has no SMTP egress, so mailbox probing is not part of the standard and any bounce-testing result must be appended as a dated column.
**Geography rule:** Houston metro first; DFW, Austin, San Antonio only to fill the quota (owner decision 2026-09-07). Expansion rows are virtual-only prospects (no in-person add-on).
**Why:** Owner asked for 100+ prospects "verified to maximum scrutiny." Directory scraping produces volume without intent; a trigger-led population plus a re-fetching verifier makes every row falsifiable and cheap to re-verify (prospect data decays in weeks). Publication-based contact evidence keeps the list defensible and inside the owner's data-honesty rules (D-006).
**Revisit if:** a deliverability service is adopted (then add an SMTP-verified gate as G5b), or first outreach shows the general-inbox rows convert too poorly to be worth verifying.
